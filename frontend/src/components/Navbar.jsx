            ...
            <div className="relative">
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-[#001845] rounded-lg shadow-lg py-1 z-50 transform opacity-100 scale-100 transition-all duration-200">
                  ...
                  <Link to="/logout" className="block px-4 py-2 text-gray-300 hover:bg-[#002466] hover:text-white transition-colors duration-200">
                    Logout
                  </Link>
                </div>
              )}
            ...
            <div className="px-2 pt-2 pb-3 space-y-1">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`block px-3 py-2 rounded-md text-base font-medium hover:text-[#4FB846] transition-colors duration-200 ${
                    isActive(link.path)
                  }`}
                >
                  {link.label}
